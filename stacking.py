...
def automl_objective(trial, X_train, y_train, cv_folds):
    ...
    try:  # Try to create and evaluate the model
        model = create_model_from_params(model_name, params)  # Create model instance from params
        mean_f1 = automl_cross_validate_model(model, X_train, y_train, cv_folds, trial)  # Cross-validate
...
def run_automl_model_search(X_train, y_train, file_path):
    """
    ...
    objective_fn = lambda trial: automl_objective(trial, X_train, y_train, AUTOML_CV_FOLDS)  # Create objective wrapper
...
def automl_stacking_objective(trial, X_train, y_train, cv_folds, candidate_models):
    """
    ...
    try:  # Try to build and evaluate stacking ensemble
        ...
        for name in selected_models:  # Build each selected base learner
            ...
            model = create_model_from_params(name, model_params)  # Create model instance
...
def run_automl_stacking_search(X_train, y_train, model_study, file_path):
    ...
    print(
        f"\n{BackgroundColors.BOLD}{BackgroundColors.GREEN}Starting AutoML stacking search with {BackgroundColors.CYAN}{AUTOML_STACKING_TRIALS}{BackgroundColors.GREEN} trials...{Style.RESET_ALL}"
    )  # Output search start message

    candidate_models = extract_top_automl_models(model_study, top_n=AUTOML_STACKING_TOP_N)  # Get top models from model search
    ...
    sampler = optuna.samplers.TPESampler(seed=AUTOML_RANDOM_STATE + 1)  # Create sampler with different seed
    ...
    objective_fn = lambda trial: automl_stacking_objective(
        trial, X_train, y_train, AUTOML_CV_FOLDS, candidate_models
    )  # Create stacking objective wrapper
    stacking_study.optimize(
        objective_fn, n_trials=AUTOML_STACKING_TRIALS, timeout=AUTOML_TIMEOUT, n_jobs=1
...
def build_automl_stacking_model(best_config):
    ...
    if meta_learner_name == "Logistic Regression":  # Logistic Regression meta-learner
        meta_model = LogisticRegression(max_iter=1000, random_state=AUTOML_RANDOM_STATE)  # Create LR
    elif meta_learner_name == "Random Forest":  # Random Forest meta-learner
        meta_model = RandomForestClassifier(n_estimators=50, random_state=AUTOML_RANDOM_STATE, n_jobs=N_JOBS)  # Create RF
    else:  # Gradient Boosting meta-learner
        meta_model = GradientBoostingClassifier(random_state=AUTOML_RANDOM_STATE)  # Create GB

    stacking_model = StackingClassifier(
        estimators=estimators,
        final_estimator=meta_model,
        cv=StratifiedKFold(
            n_splits=best_config["stacking_cv_splits"], shuffle=True, random_state=AUTOML_RANDOM_STATE
        ),
        n_jobs=N_JOBS,
...
def build_automl_results_list(best_model_name, best_params, individual_metrics, stacking_metrics, stacking_config, file_path, feature_names, n_train, n_test):
    ...
    individual_entry = {  # Build individual model result entry
        ...
        "cv_method": f"Optuna({AUTOML_N_TRIALS} trials, {AUTOML_CV_FOLDS}-fold CV)",  # CV method description
    ...
    if stacking_metrics is not None and stacking_config is not None:  # If stacking results are available
        stacking_entry = {  # Build stacking result entry
            ...
            "cv_method": f"Optuna({AUTOML_STACKING_TRIALS} trials, {AUTOML_CV_FOLDS}-fold CV)",  # CV method description
            "top_features": json.dumps(feature_names),  # Feature names as JSON
            "rfe_ranking": None,  # No RFE ranking for AutoML
...
def generate_comparison_report(results_original, results_augmented, results_merged):
    ...
    :param results_original: Dictionary of results from original data
    :param results_augmented: Dictionary of results from augmented data
    :param results_merged: Dictionary of results from merged (original+augmented) data
    :return: List of comparison result entries for CSV export
    """

    verbose_output(
        f"{BackgroundColors.GREEN}Generating data augmentation comparison report...{Style.RESET_ALL}"
    )  # Output the verbose message
    ...
    comparison_results = []  # Initialize list for comparison results

    for key in results_original.keys():  # Iterate through each result key
        orig_result = results_original[key]  # Get original result
        aug_result = results_augmented.get(key)  # Get augmented result (may be None)
        merged_result = results_merged.get(key)  # Get merged result (may be None)

        feature_set = orig_result["feature_set"]  # Extract feature set name
        model_name = orig_result["model_name"]  # Extract model name
        classifier_type = orig_result["classifier_type"]  # Extract classifier type

        orig_metrics = extract_metrics_from_result(orig_result)  # Extract original metrics
        aug_metrics = extract_metrics_from_result(aug_result) if aug_result else [0] * 7  # Extract augmented metrics or zeros
        merged_metrics = extract_metrics_from_result(merged_result) if merged_result else [0] * 7  # Extract merged metrics or zeros

        improvements = calculate_all_improvements(orig_metrics, merged_metrics)  # Calculate all improvements

        print_model_comparison(feature_set, model_name, orig_metrics, aug_metrics, merged_metrics, improvements)  # Print detailed comparison

        no_improvements = {"accuracy": 0.0, "precision": 0.0, "recall": 0.0, "f1_score": 0.0, "fpr": 0.0, "fnr": 0.0, "training_time": 0.0}  # Zero improvements for original and augmented entries

        comparison_results.append(
            build_comparison_result_entry(orig_result, feature_set, classifier_type, model_name, "Original", orig_metrics, no_improvements)
        )  # Add original result entry

        comparison_results.append(
            build_comparison_result_entry(
                orig_result, feature_set, classifier_type, model_name, "Augmented", aug_metrics, no_improvements,
                n_features_override=aug_result["n_features"] if aug_result else 0,
                n_samples_train_override=aug_result["n_samples_train"] if aug_result else 0,
                n_samples_test_override=aug_result["n_samples_test"] if aug_result else 0
            )
        )  # Add augmented result entry

        comparison_results.append(
            build_comparison_result_entry(
                orig_result, feature_set, classifier_type, model_name, "Original+Augmented", merged_metrics, improvements,
                n_features_override=merged_result["n_features"] if merged_result else 0,
                n_samples_train_override=merged_result["n_samples_train"] if merged_result else 0,
                n_samples_test_override=merged_result["n_samples_test"] if merged_result else 0
            )
        )  # Add merged result entry with improvements

    return comparison_results  # Return list of comparison results
