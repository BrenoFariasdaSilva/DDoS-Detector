...
class TrainingProgress:  # Report genuine public units or heartbeat-only activity
    ...
    def __init__(self, feature_set: Optional[str], classifier_name: str, duration_formatter: Callable[[float], str], output_stream: Optional[Any] = None, total_units: Optional[int] = None, unit_label: Optional[str] = None, heartbeat: bool = False, report_interval_seconds: float = DEFAULT_TRAINING_PROGRESS_INTERVAL_SECONDS, hyperparameters_enabled: Optional[bool] = None, augmentation_ratio: Optional[float] = None, eta_callback: Optional[Callable[[str, Optional[float]], None]] = None, resource_suffix_callback: Optional[Callable[[], str]] = None, estimated_finish_suffix_callback: Optional[Callable[[float], str]] = None, estimated_total_seconds: Optional[float] = None, local_combination_index: Optional[int] = None, local_combination_total: Optional[int] = None, previous_run_duration_label: str = "unavailable", eta_pending_run_experiments_label: str = "unavailable"):  # Initialize one training progress scope
        ...
        self.eta_pending_run_experiments_label = str(eta_pending_run_experiments_label or "unavailable")  # Store immutable pending-run duration text.
    ...
    def report_heartbeat(self) -> bool:  # Emit one active record when the task interval is due
        ...
        try:  # Keep reporting failures isolated from estimator training.
            ...
            with self.report_lock:  # Coordinate heartbeat timing with public unit callbacks for this task only.
                ...
                print(f"{self.combination_prefix}[TRAINING] Feature Set: {self.feature_set} | Classifier: {self.classifier_name}{self.combination_fields} | Status: Active | Elapsed: {elapsed_label} | ETA: {eta_label} | Previous Run Duration: {self.previous_run_duration_label} | ETA Pending Run Experiments: {self.eta_pending_run_experiments_label}{self.estimated_finish_suffix(remaining_seconds)} | PID: {os.getpid()}{self.resource_suffix()}", file=self.output_stream)  # Write contextual heartbeat with factual ETA when units exist.
    ...
    def report_unit(self, completed_units: int) -> None:  # Report one completed public training unit
        ...
        try:  # Keep progress output isolated from estimator training semantics.
            ...
            with self.report_lock:  # Coordinate callback timing with an optional heartbeat for this task only.
                ...
                print(f"{self.combination_prefix}[TRAINING] Feature Set: {self.feature_set} | Classifier: {self.classifier_name}{self.combination_fields} | Status: Active | Elapsed: {elapsed_label} | ETA: {eta_label} | Previous Run Duration: {self.previous_run_duration_label} | ETA Pending Run Experiments: {self.eta_pending_run_experiments_label}{self.estimated_finish_suffix(remaining_seconds)} | PID: {os.getpid()}{self.resource_suffix()}", file=self.output_stream)  # Write one active ETA row for normal in-progress output.
