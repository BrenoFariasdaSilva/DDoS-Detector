...
def run_rfe_cv(csv_path, X_numeric, y_array, feature_columns, hyperparameters):
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(skf.split(X_train_scaled, y_train_array), start=1):
        ...
        send_telegram_message(
            TELEGRAM_BOT,
            f"RFE: Finished fold {fold_idx}/{n_splits} for dataset {Path(csv_path).stem} in {calculate_execution_time(0, metrics_tuple[6])} with F1: {truncate_value(metrics_tuple[3])}"
        )
