...
def run_pca_analysis(csv_path, n_components_list=[8, 16, 24, 32, 48], parallel=True, max_workers=None):
	...
	if parallel and len(n_components_list) > 1: # If parallel execution is enabled and multiple configurations
		try: # Attempt parallel execution
			...
			print(f"{BackgroundColors.GREEN}Running {BackgroundColors.CYAN}PCA Analysis{BackgroundColors.GREEN} in Parallel with {BackgroundColors.CYAN}{workers}{BackgroundColors.GREEN} Worker(s)...{Style.RESET_ALL}")
...
def main():
	...
	n_components_list = [8, 16, 32] # List of PCA component counts to test
