    ...
    try:
        ...
        try:  # Attempt to extract Pareto front
            for candidate in population:  # Iterate over each individual in population
                ...
                is_dominated = False  # Flag to verify if candidate is dominated
                for other in population:  # Compare candidate with all other individuals
                    if not other.fitness.valid or candidate is other:  # Skip invalid or same individual
                        continue  # Move to next individual
                    if tools.emo.isDominated(candidate.fitness.values, other.fitness.values):  # Verify if candidate is dominated by other
...
def plot_ga_convergence(
    ...
    try:
        ...
        try:  # Try to plot and save the figure
            ...
            ensure_figure_min_4k_and_save(fig=plt.gcf(), path=fig_path, dpi=300)  # Save figure to file with 150 DPI resolution
...
def generate_individual_convergence_plots(history_data, generations, base_dataset_name, run, pop_size, plot_output_dir):
    ...
    Produces up to 7 single-metric convergence plots and one dual-axis multi-objective plot.

    :param history_data: Dict containing all tracking histories.
    :param generations: List of generation numbers (1..n_gens).
    :param base_dataset_name: Sanitized dataset name for plot titles and filenames.
    :param run: Run index (1-based).
    :param pop_size: Population size used in this run.
    :param plot_output_dir: Directory to save plot files into.
    ...
    try:
        ...
        return saved_plots  # Return list of all saved convergence plot paths
    except Exception as e:  # Catch any exception to ensure logging and Telegram alert
        print(str(e))  # Print error to terminal for server logs
        send_exception_via_telegram(type(e), e, e.__traceback__)  # Send full traceback via Telegram
        raise  # Re-raise to preserve original failure semantics
...
def generate_convergence_plots(
    ...
    try:
        ...
        try:  # Attempt to generate all convergence plots
            n_gens = max(
                len(history_data.get("best_f1", [])),
                len(history_data.get("best_features", [])),
                len(history_data.get("avg_f1", [])),
                len(history_data.get("avg_features", [])),
                len(history_data.get("pareto_size", [])),
                len(history_data.get("hypervolume", [])),
                len(history_data.get("diversity", [])),
            ...
            saved_plots = generate_individual_convergence_plots(
                history_data, generations, base_dataset_name, run, pop_size, plot_output_dir
            )  # Generate all individual convergence plots and collect saved paths
...
def plot_comparison_overlay_f1(all_runs_data, comparison_dir, base_dataset_name, dataset_name, total_runs):
    """
    Generate overlay line plot of best F1-score convergence across all runs.

    :param all_runs_data: List of run data dictionaries with history_data, pop_size, run_id.
    :param comparison_dir: Output directory for comparison plots.
    :param base_dataset_name: Sanitized dataset name for filename construction.
    :param dataset_name: Display name of the dataset for plot titles.
    :param total_runs: Total number of runs for title annotation.
    :return: Path to saved plot, or None if generation failed.
    """
...
def plot_comparison_distributions(all_runs_data, comparison_dir, base_dataset_name, dataset_name, total_runs):
    """
    Generate distribution histograms for final F1-scores, feature counts, and hypervolumes.

    :param all_runs_data: List of run data dictionaries with final_f1, final_features, and history_data.
    :param comparison_dir: Output directory for comparison plots.
    :param base_dataset_name: Sanitized dataset name for filename construction.
    :param dataset_name: Display name of the dataset for plot titles.
    :param total_runs: Total number of runs for title annotation.
    :return: List of paths to saved distribution plots.
    """

    try:
        saved = []  # List to collect paths of saved distribution plots

        try:  # Try to create F1-score distribution histogram
            ...
            ensure_figure_min_4k_and_save(fig=plt.gcf(), path=plot_path, dpi=300)  # Save figure
            plt.close()  # Close plot
            saved.append(plot_path)  # Add to saved plots list
        except Exception:  # If plotting fails
            plt.close()  # Close plot

        try:  # Try to create feature count distribution histogram
        ...
        try:  # Try to create hypervolume distribution histogram
        ...
        return saved  # Return all saved distribution plot paths
    except Exception:  # If overall distribution plotting fails
        return []  # Return empty list on failure


def plot_comparison_per_run_bars(all_runs_data, comparison_dir, base_dataset_name, dataset_name, total_runs):
    """
    Generate per-run bar charts for final F1-scores, feature counts, and hypervolumes.

    :param all_runs_data: List of run data dictionaries with final_f1, final_features, pop_size, run_id, and history_data.
    :param comparison_dir: Output directory for comparison plots.
    :param base_dataset_name: Sanitized dataset name for filename construction.
    :param dataset_name: Display name of the dataset for plot titles.
    :param total_runs: Total number of runs for title annotation.
    :return: List of paths to saved bar chart plots.
    """

    try:
        saved = []  # List to collect paths of saved bar chart plots

        try:  # Try to create F1-score bar chart with quartile coloring
        ...
        try:  # Try to create feature count bar chart
            run_labels = [f"P{run['pop_size']}_R{run['run_id']}" for run in all_runs_data]  # Create run labels
            final_features = [run["final_features"] for run in all_runs_data]  # Extract final feature counts
        ...
        try:  # Try to create hypervolume bar chart
        ...
        return saved  # Return all saved bar chart plot paths
    except Exception:  # If overall bar chart plotting fails
        return []  # Return empty list on failure


def plot_comparison_aggregated_convergence(all_runs_data, comparison_dir, base_dataset_name, dataset_name):
    """
    Generate aggregated mean plus/minus standard deviation convergence plot for best F1-scores.

    :param all_runs_data: List of run data dictionaries with history_data containing best_f1 lists.
    :param comparison_dir: Output directory for comparison plots.
    :param base_dataset_name: Sanitized dataset name for filename construction.
    :param dataset_name: Display name of the dataset for plot titles.
    :return: Path to saved plot, or None if generation failed.
    ...
    try:
        ...
        if not all_f1_histories or max_gens == 0:  # If no F1 histories collected
...
def plot_comparison_scatter_and_correlations(all_runs_data, comparison_dir, base_dataset_name, dataset_name, total_runs):
    """
    Generate scatter plot of F1 vs features and correlation heatmap across runs.

    :param all_runs_data: List of run data dictionaries with final_f1, final_features, pop_size, and history_data.
    :param comparison_dir: Output directory for comparison plots.
    :param base_dataset_name: Sanitized dataset name for filename construction.
    :param dataset_name: Display name of the dataset for plot titles.
    :param total_runs: Total number of runs for title annotation.
    :return: List of paths to saved scatter and correlation plots.
    """

    try:
        saved = []  # List to collect paths of saved plots
        ...
        return saved  # Return all saved scatter and correlation plot paths
    except Exception:  # If overall scatter/correlation plotting fails
        return []  # Return empty list on failure


def plot_comparison_boxplot_by_pop(all_runs_data, comparison_dir, base_dataset_name, dataset_name, min_pop, max_pop):
    """
    Generate boxplot of F1-score distribution grouped by population size.

    :param all_runs_data: List of run data dictionaries with final_f1 and pop_size.
    :param comparison_dir: Output directory for comparison plots.
    :param base_dataset_name: Sanitized dataset name for filename construction.
    :param dataset_name: Display name of the dataset for plot titles.
    :param min_pop: Minimum population size tested.
    :param max_pop: Maximum population size tested.
    :return: Path to saved plot, or None if generation failed or only one population size tested.
    """

    try:
        if max_pop <= min_pop:  # If only one population size tested
...
def print_comparison_plots_summary(saved_plots, comparison_dir):
    """
    Print a formatted summary of all multi-run comparison plots generated.

    :param saved_plots: List of paths to all saved comparison plots.
    :param comparison_dir: Directory where plots were saved.
    :return: None
    """

    try:
        if not saved_plots:  # If no plots were saved
        ...
        print(f"\n{BackgroundColors.GREEN}{'='*80}{Style.RESET_ALL}")  # Print top separator line
        print(f"{BackgroundColors.GREEN}Multi-Run Visualization Summary{Style.RESET_ALL}")  # Print summary header
        print(f"{BackgroundColors.GREEN}{'='*80}{Style.RESET_ALL}")  # Print middle separator line
        print(f"{BackgroundColors.GREEN}Total Plots Generated: {BackgroundColors.CYAN}{len(saved_plots)}{Style.RESET_ALL}")  # Print plot count
        print(f"{BackgroundColors.GREEN}Output Directory: {BackgroundColors.CYAN}{comparison_dir}{Style.RESET_ALL}")  # Print output directory
        print(f"{BackgroundColors.GREEN}{'='*80}{Style.RESET_ALL}\n")  # Print bottom separator line
    except Exception:  # If printing fails
        pass  # Silently ignore summary output failures
...
def generate_multi_run_comparison_plots(results_dict, csv_path, dataset_name, min_pop, max_pop, n_generations, cxpb, mutpb):
    ...
    try:
        ...
        try:  # Attempt to generate all comparison plots
            ...
            overlay_path = plot_comparison_overlay_f1(all_runs_data, comparison_dir, base_dataset_name, dataset_name, total_runs)  # Generate overlay F1-score convergence plot
            if overlay_path:  # If overlay plot was saved successfully
                saved_plots.append(overlay_path)  # Add overlay plot to saved list

            saved_plots.extend(plot_comparison_distributions(all_runs_data, comparison_dir, base_dataset_name, dataset_name, total_runs))  # Generate and collect distribution histogram plots

            saved_plots.extend(plot_comparison_per_run_bars(all_runs_data, comparison_dir, base_dataset_name, dataset_name, total_runs))  # Generate and collect per-run bar chart plots

            agg_path = plot_comparison_aggregated_convergence(all_runs_data, comparison_dir, base_dataset_name, dataset_name)  # Generate aggregated convergence plot
            if agg_path:  # If aggregated convergence plot was saved successfully
                saved_plots.append(agg_path)  # Add aggregated plot to saved list

            saved_plots.extend(plot_comparison_scatter_and_correlations(all_runs_data, comparison_dir, base_dataset_name, dataset_name, total_runs))  # Generate and collect scatter and correlation plots

            boxplot_path = plot_comparison_boxplot_by_pop(all_runs_data, comparison_dir, base_dataset_name, dataset_name, min_pop, max_pop)  # Generate boxplot by population size
            if boxplot_path:  # If boxplot was saved successfully
                saved_plots.append(boxplot_path)  # Add boxplot to saved list

            print_comparison_plots_summary(saved_plots, comparison_dir)  # Print formatted summary of generated plots
...
def plot_feature_usage_all_features(feature_run_counts: dict, total_runs: int, feature_names: list, comparison_dir: str, base_dataset_name: str, dataset_name: str) -> str | None:
    ...
    try:
        if feature_names is None or len(feature_names) == 0 or total_runs == 0:  # Verify that feature names and runs are available
            return None  # Return None when required data is missing

        all_counts = {f: feature_run_counts.get(f, 0) for f in feature_names}  # Build counts dict ensuring every dataset feature is present

        features_sorted = sorted(all_counts, key=lambda f: all_counts[f], reverse=True)  # Sort all features descending by usage count
        counts_sorted = [all_counts[f] for f in features_sorted]  # Extract counts in sorted order
        pcts_sorted = [(c / total_runs) * 100 for c in counts_sorted]  # Compute usage percentages in sorted order

        fig_height = max(6, len(features_sorted) * 0.35)  # Compute dynamic figure height based on feature count
        fig, ax = plt.subplots(figsize=(14, fig_height))  # Create figure with dynamic height

        bar_colors = ["#d62728" if c == 0 else "#1f77b4" for c in counts_sorted]  # Assign red to unused features and blue to used features
        y_pos = list(range(len(features_sorted)))  # Generate y-axis positions for bars
        bars = ax.barh(y_pos, pcts_sorted, color=bar_colors, alpha=0.85, edgecolor="black", linewidth=0.5)  # Draw horizontal bars with per-bar colors
        ...
        ax.set_xlabel("Percentage of Runs Using Feature (%)", fontsize=12)  # Set X-axis label
        ax.set_ylabel("Feature", fontsize=12)  # Set Y-axis label
        ax.set_title(f"Feature Usage Across Runs (All Features)\n{dataset_name} ({total_runs} runs)", fontsize=14)  # Set plot title
        ax.set_xlim(0, 115)  # Extend X limit to leave space for bar labels
        ax.grid(True, linestyle="--", alpha=0.3, axis="x")  # Add X-axis grid lines

        plt.tight_layout()  # Adjust layout to prevent clipping

        plot_path = os.path.join(comparison_dir, f"{base_dataset_name}_feature_usage_all_features.png")  # Construct full output path
        ensure_figure_min_4k_and_save(fig=plt.gcf(), path=plot_path, dpi=300)  # Save figure at minimum 4K resolution
        plt.close()  # Close figure to free memory
...
def analyze_top_features(df, y, top_features, csv_path="."):
    ...
    try:
        ...
        for feature in top_features:  # For each top feature
            ...
            plt.tight_layout()  # Adjust layout
            ensure_figure_min_4k_and_save(fig=plt.gcf(), path=f"{output_dir}/{base_dataset_name}-{safe_filename(feature)}.png")  # Save the plot
            plt.close()  # Close the plot to free memory
    except Exception as e:  # Catch any exception to ensure logging and Telegram alert
        print(str(e))  # Print error to terminal for server logs
        send_exception_via_telegram(type(e), e, e.__traceback__)  # Send full traceback via Telegram
